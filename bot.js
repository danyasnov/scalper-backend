        ...
        return Extra
            ...
            .markup((m) => {
                ...
                tasks.forEach((task, i) => {
                    btnArray = [...btnArray, m.callbackButton(`BTC-${task.currency} ${getBookType(task.bookType)} ${task.interval}m R${task.priceRange}% F${task.filterValue}${task.filterType === 0 ? '%' : 'BTC'} (${task.active ? 'online' : 'stopped'})`, `${action}-task:${task._id}`)]
                });
